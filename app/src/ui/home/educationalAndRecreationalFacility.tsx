...
const EducationalAndRecreationalFacilityType = ({ admin }: { admin?: boolean }) => {
    ...
    const handleClick = (identifier: string) => redirectClient(`/reservation/${identifier}`)
        ...
        <>
            {
                isLoading
                    ...
                    : educationalTypeData?.data.map((educationalType) => (
                        ...
                        hover:bg-slate-100 hover:cursor-pointer border border-slate-2001" onClick={() => handleClick(educationalType.identifier)}>
